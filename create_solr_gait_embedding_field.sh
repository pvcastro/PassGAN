...
curl -X POST -H 'Content-type:application/json' --data-binary '{"add-field":{"name":"gait_embedding_1024","type":"embedding_1024","stored":true }}' http://localhost:8983/solr/gait/schema
...
curl -X POST -H 'Content-type:application/json' --data-binary '{"add-field":{"name":"gait_embedding_512","type":"embedding_512","stored":true }}' http://localhost:8983/solr/gait/schema
...
curl -X POST -H 'Content-type:application/json' --data-binary '{"add-field":{"name":"gait_embedding_128","type":"embedding_128","stored":true }}' http://localhost:8983/solr/gait/schema
